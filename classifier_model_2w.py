...
class UModel:
    ...
    def load_weights(self, weights_path):
        """
        Load the saved weights from the source model and transfer them to the target model.

        Args:
            weights_path (str): Path to the saved weights file.
        """
        # Load the saved weights from the source model
        source_weights = torch.load(weights_path)

        # Get the target model and its state dictionary
        target_model = self.model
        target_state_dict = target_model.state_dict()

        # Transfer weights from source model to target model layer by layer
        for target_name, target_param in target_state_dict.items():
            if target_name in source_weights:
                target_param.copy_(source_weights[target_name])
